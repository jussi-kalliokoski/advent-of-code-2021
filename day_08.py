...
for k, v in SEGMENTS_BY_DIGIT.items():
    if len(v) not in DIGITS_BY_SEGMENT:
        DIGITS_BY_SEGMENT[len(v)] = []
    DIGITS_BY_SEGMENT[len(v)] += [k]
DIGITS_BY_SEGMENT = { k: DIGITS_BY_SEGMENT[k] for k in sorted(DIGITS_BY_SEGMENT) }

def __parse(text: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
    lines = tf.strings.split(tf.strings.strip(text), "\n")
    by_io = tf.strings.split(lines, " | ")
    segments = tf.strings.split(by_io, " ").to_tensor()
    def without_empty(strs):
        indices = tf.reshape(tf.where(strs != ""), [-1])
        return tf.map_fn(
            lambda idx: strs[idx],
            indices,
            dtype=tf.string,
        )
    input_segments = tf.map_fn(without_empty, segments[:, 0])
    output_segments = tf.map_fn(without_empty, segments[:, 1])
    return input_segments, output_segments

def __count_known_segments_in_results(text: tf.Tensor) -> tf.Tensor:
    input_segments, output_segments = __parse(text)
    unique_lengths = tf.constant([k for k, v in DIGITS_BY_SEGMENT.items() if len(v) == 1])
    lengths = tf.reshape(tf.strings.length(output_segments), [-1])
    lengths = tf.map_fn(
        lambda idx: lengths[idx],
        tf.where(lengths > 0)[:, 0],
        dtype=tf.int32,
    )
    identifiable_digits = tfutils.reduce_fn(
        lambda current, l: tf.where(lengths == l, tf.fill(tf.shape(lengths), 1), current),
        unique_lengths,
        tf.fill(tf.shape(lengths), 0),
    )
    return tf.math.reduce_sum(identifiable_digits)

def __get_results_sum(text: tf.Tensor) -> tf.Tensor:
    input_segments, output_segments = __parse(text)
    """
    given:
    * every digit appears once in every input set
    * segments e,b,f have unique numbers of occurrences in the digits (4,6,9 respectively)
    * digits 1,4,7,8 have unique segment counts (2,4,3,7 respectively)
    we can:
    * group the characters by number of occurrences in the inputs
    * e = grouped[4]
    * b = grouped[6]
    * f = grouped[9]
    * sort the inputs ascending by length
    * removing known characters from inputs before each of the following:
        - c = inputs[0]
        - a = inputs[1]
        - d = inputs[2]
        - g = inputs[3]
    """
    def digit_bucket(chars):
        return tf.reduce_sum(tf.map_fn(
            lambda c: tf.bitwise.left_shift(tf.constant(1, dtype=tf.int64), c),
            chars,
        ))

    def digits_to_number(digits):
        return tf.reduce_sum(tf.map_fn(
            lambda idx: (tf.constant(10, dtype=tf.int64) ** (tf.shape(digits, out_type=tf.int64)[0] - idx - 1)) * digits[idx],
            tf.range(0, tf.shape(digits, out_type=tf.int64)[0]),
        ))

    default_order = tf.constant([c for c in "abcdefg"])

    lookup = tf.fill([128], tf.constant(-1, dtype=tf.int64))
    for digit, segments in SEGMENTS_BY_DIGIT.items():
        b = digit_bucket(tf.constant(["abcdefg".index(c) for c in segments], dtype=tf.int64))
        lookup = tf.concat([lookup[:b], lookup[b:b+1] + digit + 1, lookup[b+1:]], 0)

    def map_output(idx):
        inputs = input_segments[idx]

        def find(segment):
            nonlocal inputs
            inputs = tf.strings.regex_replace(inputs, segment, "")
            return tf.where(default_order == segment)[0][0]

        lengths = tf.strings.length(inputs)
        chars = tf.reshape(tf.strings.bytes_split(tf.strings.reduce_join(inputs)), [-1])
        occurrences = tf.map_fn(
            lambda c: tf.shape(tf.where(chars == c), out_type=tf.int64)[0],
            default_order,
            dtype=tf.int64,
        )

        e = find(default_order[tf.where(occurrences == 4)[0][0]])
        b = find(default_order[tf.where(occurrences == 6)[0][0]])
        f = find(default_order[tf.where(occurrences == 9)[0][0]])
        c = find(inputs[tf.where(lengths == 2)[0][0]])
        a = find(inputs[tf.where(lengths == 3)[0][0]])
        d = find(inputs[tf.where(lengths == 4)[0][0]])
        g = find(inputs[tf.where(lengths == 7)[0][0]])

        order = tf.stack([a, b, c, d, e, f, g])
        mapping = tf.map_fn(
            lambda i: tf.where(order == i)[0][0],
            tf.range(0, tf.shape(order, out_type=tf.int64)[0]),
        )

        def segments_to_digit(segments):
            b = digit_bucket(tf.map_fn(
                lambda segment: mapping[tf.where(default_order == segment)[0][0]],
                tf.strings.bytes_split(segments),
                dtype=tf.int64,
            ))
            return lookup[b]

        output_digits = tf.map_fn(
            segments_to_digit,
            output_segments[idx],
            dtype=tf.int64,
        )
        return digits_to_number(output_digits)

    return tf.reduce_sum(tf.map_fn(map_output, tf.range(0, tf.shape(input_segments, out_type=tf.int64)[0])))

assert __count_known_segments_in_results(EXAMPLE_INPUT) == 26
assert __count_known_segments_in_results(PUZZLE_INPUT) == 387
print("Part 1:", __count_known_segments_in_results(PUZZLE_INPUT).numpy())
assert __get_results_sum(EXAMPLE_INPUT) == 61229
assert __get_results_sum(PUZZLE_INPUT) == 986034
print("Part 2:", __get_results_sum(PUZZLE_INPUT).numpy())
